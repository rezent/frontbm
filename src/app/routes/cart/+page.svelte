<script lang="ts">
  import {
    ...
    getItemKey,
    type CartItem,
  } from '@entities/cart'; // ← ИСПРАВЛЕНО: используем FSD store
  ...
  //   const option = item.options[optionType];
  //   const selectedValue = item.selectedOptions[optionType];
  //   if (!option || !selectedValue) return '';
  //   const optionObj = option.find(opt => opt.id === selectedValue);
  //   return optionObj ? optionObj.name : selectedValue;
  // }
