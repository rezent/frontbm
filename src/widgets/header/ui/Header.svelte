<script lang="ts">
  import { cart, cartCount, cartTotal, cartActions, getItemKey } from '@entities/cart';
  // import { modalActions } from '@features/callback-modal'; // Пока не используется
  import type { CartItem } from '@shared/types';
  import { formatPrice } from '@shared/lib/helpers';
  import { onMount, onDestroy } from 'svelte';
  import { goto } from '$app/navigation';
  import CallbackModal from '@features/callback-modal/ui/CallbackModal.svelte';
        ...
        <div class="hidden min-[479px]:flex items-center justify-end space-x-4">
          {#if isLoggedIn}
            ...
            <div class="relative dropdown">
              ...
              >
                ...
                <span>{userName}</span>
                <svg
                  class="w-3 h-3 transform transition-transform duration-200 {showUserMenu
                    ? 'rotate-180'
                    : 'rotate-0'}"
                  fill="currentColor"
                  viewBox="0 0 20 20"
              ...
              {#if showUserMenu}
                <div
                  class="absolute top-full right-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50 text-gray-700"
                >
                  <a
                    href="/profile"
          ...
          {:else}
            ...
            <div class="flex items-center space-x-4">
              ...
              <a href="/register" class="hover:text-gray-300 transition-colors"> Регистрация </a>
            </div>
          {/if}
        </div>
      </div>
        ...
        <div class="flex items-center">
          <a href="/" class="group flex items-center">
            <img
              ...
              alt="ООО БМ ФИЛЬТР - Промышленные фильтры логотип"
        ...
        <nav class="hidden lg:flex items-center space-x-1">
          ...
          <div class="relative dropdown">
            ...
            >
              ...
              <span>Каталог</span>
              <svg
                class="w-4 h-4 transform transition-transform duration-200 {showCatalogMenu
                  ? 'rotate-180'
                  : 'rotate-0'}"
                fill="currentColor"
                viewBox="0 0 20 20"
            ...
            {#if showCatalogMenu}
              <div
                class="absolute top-full left-0 mt-2 w-96 bg-white rounded-2xl shadow-xl border border-gray-100 py-6 z-50"
              >
                ...
                <div class="px-6 py-4 grid grid-cols-1 gap-2">
                  <a
                    href="/catalog/filtroelements"
                    class="group flex items-center space-x-4 p-4 rounded-xl hover:bg-gradient-to-r hover:from-gray-50 hover:to-gray-100 transition-all duration-200"
                  >
                    <div
                      class="w-12 h-12 rounded-xl bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center group-hover:scale-110 transition-transform"
                    >
                    ...
                    <div class="flex-1">
                      <div class="font-semibold text-gray-900 group-hover:text-gray-700">
                        Фильтроэлементы
                      </div>
                      <div class="text-sm text-gray-500">
                        Аналоги PALL, Atlas Copco, собственного производства
                      </div>
                      <div class="text-xs text-gray-600 font-medium mt-1">5 позиций</div>
                    </div>
                  </a>
                  <a
                    href="/catalog/filterholders"
                    class="group flex items-center space-x-4 p-4 rounded-xl hover:bg-gradient-to-r hover:from-gray-50 hover:to-gray-100 transition-all duration-200"
                  >
                    <div
                      class="w-12 h-12 rounded-xl bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center group-hover:scale-110 transition-transform"
                ...
                <div class="px-6 pt-4 border-t border-gray-100">
                  <a
                    href="/catalog"
                    class="block w-full text-center bg-sky-600 hover:bg-sky-700 text-white px-4 py-3 rounded-xl font-semibold transition-all duration-200 transform hover:scale-105"
                  >
                    📋 Весь каталог
                  </a>
                </div>
          ...
          <div class="relative dropdown">
            ...
            >
              <span>Страницы</span>
              <svg
                class="w-4 h-4 transform transition-transform duration-200 {showPagesMenu
                  ? 'rotate-180'
                  : 'rotate-0'}"
                fill="currentColor"
                viewBox="0 0 20 20"
        ...
        <div class="flex items-center space-x-3">
          ...
          <div class="hidden md:block relative">
            <input
              type="text"
              placeholder="Поиск..."
              class="w-64 pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all duration-200"
            />
            <svg
          ...
          >
            <button
              class="relative p-2.5 rounded-xl text-gray-700 hover:text-sky-600 hover:bg-blue-50 transition-all duration-200 cursor-pointer border-0 bg-transparent"
              on:click={() => goto('/cart')}
              ...
              >
                ...
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
                  <div class="flex items-center justify-between">
                    ...
                    <span class="text-sm text-gray-500"
                      >{$cartCount}
                      {$cartCount === 1 ? 'товар' : $cartCount < 5 ? 'товара' : 'товаров'}</span
                    >
                  </div>
                </div>
                ...
                <div class="max-h-80 overflow-y-auto">
                  {#each $cart.slice(0, 5) as item (getItemKey(item))}
                    <div
                      class="flex items-center space-x-4 p-4 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0"
                    >
                      <!-- Product Image -->
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center justify-between mt-2">
                          <span class="text-sm font-medium text-gray-900"
                            >{formatPrice(getTotalItemPrice(item))}</span
                          >
                ...
                <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                  ...
                  <button
                    ...
                    class="w-full bg-sky-600 hover:bg-sky-800 text-white py-3 rounded-xl font-semibold transition-all duration-200 transform hover:scale-105 shadow-lg hover:shadow-xl"
                  >
                    Перейти в корзину
                  </button>
                </div>
              ...
              >
                <div class="text-center">
                  ...
                  <button
                    ...
                    class="bg-gradient-to-r from-sky-600 to-sky-700 hover:from-sky-700 hover:to-sky-800 text-white px-6 py-2.5 rounded-xl font-semibold transition-all duration-200 transform hover:scale-105"
                  >
                    Перейти в каталог
                  </button>
                </div>
          ...
          <button
            ...
            class="lg:hidden p-2.5 rounded-xl text-gray-700 hover:text-sky-600 hover:bg-sky-50 transition-all duration-200"
            aria-label="Меню"
          >
    ...
    >
      <div class="container mx-auto px-4 py-6">
        ...
        <div class="mt-6 pt-6 border-t border-gray-200">
          <button
            on:click|stopPropagation={openCallbackModal}
            class="w-full bg-gradient-to-r from-sky-700 to-sky-800 hover:from-sky-800 hover:to-sky-900 text-white py-4 rounded-xl font-semibold shadow-lg transform hover:scale-105 transition-all duration-200"
          >
            Заказать звонок
          </button>
        </div>
